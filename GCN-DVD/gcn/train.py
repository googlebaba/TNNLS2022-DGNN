...
flags.DEFINE_integer('early_stopping', 100, 'Tolerance for early stopping (# of epochs).') #citeseer 20iters, cora,pubmed 10
...
for epoch in range(FLAGS.epochs):
    ...
    #row, col = np.diag_indices_from(cor_mat)
    #cor_mat[row, col] = 0
    
    #print("cor_mat", cor_mat.shape, np.eye(cor_mat.shape[0]))
    #cor_mat[np.eye(cor_mat.shape[0])] = 0
    #print("correlation", np.linalg.norm(cor_mat, ord=2))
