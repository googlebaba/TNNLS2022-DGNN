...
def mask2id(mask):
    ...
    for n in range(len(mask)):
        if mask[n]:
            idx.append(n)
    return idx
def count_biased_data(dataset):
    #adj, features, y_train, y_val, y_test, train_mask, val_mask, test_mask, labels = load_data(dataset)

    _, _, _, _, _, _, _, _,  labels = load_data('cora')
    adj, features, y_train, y_val, y_test, train_mask, val_mask, test_mask = load_biased_data(dataset)
    print("train_mask", train_mask.shape)
    label = np.argmax(labels, axis=1)
    print("label shape", labels.shape)
    count_class = {}
    ...
    for i in test_idx:
        ...
        count_test1.append(diff)
        if same == 0:
            tmp = 0
        else:
            tmp = diff/same
        count_test2.append(tmp)

    print("train")
    print(pd.DataFrame(count_list2).describe())
    print("test")
    print(pd.DataFrame(count_test2).describe())
    return adj, features, y_train, y_val, y_test, train_mask, val_mask, test_mask

#count_biased_data('cora_big')
