...
placeholders = {
    ...
    'P_in': tf.placeholder(tf.float32, shape=([None])),
...
def run_GCN(lambda1, lambda2):
 ...
 #       feed_dict_val.update({placeholders['adjs']: np.array(adj.astype(np.int32))})
    ...
    for epoch in range(0):
        ...
        outs = sess.run([model.opt_op, model.loss, model.accuracy, model.weight], feed_dict=feed_dict)
        for _ in range(1):
    ...
    for epoch in range(FLAGS.epochs):
        if epoch % 5 ==0:
            alpha = sess.run(model.trans_weight, feed_dict=feed_dict)
            P = compute_P(alpha)          
            feed_dict.update({placeholders['P_in']: P})
...
#        feed_dict.update({placeholders['adjs']: np.array(train_adj.astype(np.float32))})
        ...
        #print("alpha", outs[-1])
        #print("weight", outs[-1])
#        hidden_embedding = outs[-2]
#        hidden_embedding = hidden_embedding[train_mask, :]
#        print("shape", hidden_embedding.shape)
#        cor_mat = np.cov(hidden_embedding.T, aweights=outs[-3]*outs[-3])
...
#        print("cov**************", n2)
        ...
        for _ in range(1):
            outsb = sess.run([model.opt_opb1, model.lossb1, model.accuracy], feed_dict=feed_dict)
...
if __name__ == '__main__':
    n = 10
    para1 = [100, 10, 1, 0.1, 0.01]
    para2 = [100, 10, 1, 0.1, 0.01]
    best_lambda1 = 0
    best_lambda2 = 0
    best_acc = 0
    str1 = "cora new test"
